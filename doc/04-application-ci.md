...
6. **業務におけるMatrixテストの使い所（Webアプリ vs ライブラリ）**:
   - ユーザーの疑問: 「業務でわざわざ3つのバージョンでテストを回すのはどんな時？普通のWebアプリなら本番バージョンは1つでは？」
   - 実際:
     - **Webアプリ開発**: ユーザーの指摘通り、ECSやK8s等で動く単一Webアプリなら本番と同じ単一バージョン（`go-version-file: 'go.mod'`）のみテストするのが最も効率的で無駄がない。
     - **Matrixの主用途**:
       1. **OSS / 社内共通ライブラリ / SDK**: 利用者側（別サービス）のGoバージョンがまちまちなため、サポート対象の複数マイナーバージョン（1.21, 1.22, 1.23）で壊れていないかを保証する。
       2. **ランタイム移行期**: 本番を1.22から1.23へ上げる移行期間中、PRで両方の互換性を担保する。
       3. **CLIツール等のマルチOS・マルチArch対応**: `[ubuntu, macos, windows]` や `[amd64, arm64]` を掛け合わせて動作保証する。
     - なぜパッチではなくマイナーなのか？: Go言語では「Go 1」が維持されており、マイナー（1.21, 1.22）が他言語のメジャー更新に相当するため。パッチ（1.22.1 → 1.22.2）は後方互換100%のため複数回す意味が薄い。
...
- **`actions/checkout` を明示しないとリポジトリが存在しない**:
  - GitLab CI/CD ではジョブ開始時に自動でコードが clone されるため、GitHub Actions でも自動だと勘違いしやすい。Stepで `actions/checkout@v4` を明示しないとワークスペースは空っぽになる。
- **Runnerには最初からGoが入っているが故の油断**:
  - `ubuntu-latest` には主要バージョンのGoがプリインストールされているため、`actions/setup-go` を書かなくても `go test` が一見動いてしまう。しかしRunnerイメージ更新によるバージョン勝手上がりや、プロジェクトの `go.mod` との乖離を防ぐため、明示的に `setup-go` を書くことが実務上不可欠である。
- **Job分割による初期化ボイラープレートの必然性**:
  - Jobを `lint`, `test`, `build` に分けると、Jobごとに独立したクリーンRunnerが起動するため、全Jobで毎回 `actions/checkout` と `actions/setup-go` を書く必要がある（理想は1回で引き継ぎたいが、完全並列性とのトレードオフ）。
- **`fail-fast: true` によるMatrixの道連れキャンセル**:
  - 並列で別マシンで動いているため他方も最後まで完走すると予想しがちだが、GitHub Actionsのデフォルトは `fail-fast: true`。1つでも落ちると実行中の他バージョンも即座に強制終了（`cancelled`）される。全バージョンの結果を見たい場合は `fail-fast: false` が必須。

## ブログへ残したい要点

- **環境準備のアプローチの違い（コンテナイメージ vs VM + Setup Action）**:
  - GitLab CI/CDは `image: golang:1.22` でDockerコンテナを直接指定する文化だが、GitHub Actionsは `ubuntu-latest` などのVM Runner上で公式の `actions/setup-*` を使って言語環境をセットアップするのが標準。
- **`go-version-file: 'go.mod'` による真実の単一情報源（Single Source of Truth）**:
  - CIのYAMLにバージョン番号（`1.22`）をハードコードせず、`go-version-file` を使ってコード側の定義を参照させることで、バージョン乖離の事故を防ぎ、`go.mod` の更新だけでCIも追従させることができる。
- **DAGによる実務CIの組み立て**:
  - `lint` と `test` を並列実行し、両方の成功を待ってから `build` を動かす（`needs: [lint, test]`）ことで、ステージ構文なしでシンプルかつ高速なCIを構築できる。
- **Matrixテストの使い所と `fail-fast` の設計思想**:
  - 自社運用の単一Webアプリであれば、Matrixで複数バージョンを回す必要はなく、本番と同じ単一バージョン（`go.mod`準拠）でテストするのが最もリソース効率が良い。
  - 一方でOSSや社内共通ライブラリ、言語移行期、マルチOS対応では `strategy.matrix` が絶大な威力を発揮する。
  - GitHub Actionsは実行時間課金のため、デフォルトでリソース節約を優先する `fail-fast: true`（1つコケたら即座に他をキャンセル）になっている思想を理解し、必要に応じて `fail-fast: false` を使い分ける。
