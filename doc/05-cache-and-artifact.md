...
3. **キャッシュのイミュータブル性（不変性）とライブラリバージョンの固定**:
   - **キャッシュは同じキーで上書きできない**: 一度キーが作成されると、GitHub Actionsではそのキーの中身を上書き更新することはできない。
   - **なぜライブラリのバージョン固定（ロックファイル）が必須なのか？**:
     - `go.mod` や `package.json` でバージョンをレンジ指定（`^1.0.0` 等）していると、設定ファイルは変わらない（＝キーのハッシュ値も変わらない）のに外部レジストリで新バージョンが公開される。
     - すると「キーが同じだからキャッシュヒット！」となり、古い依存関係が永久に使われ続ける（キャッシュの腐敗）。
     - そのため、厳密なバージョンとチェックサムが記録されたロックファイル（`go.sum`, `package-lock.json` 等）を `hashFiles()` の対象にすることが鉄則。
   - **強制リフレッシュ（force）したい場合**:
     - ① キャッシュキーにプレフィックス（`v1-`, `v2-`）を付けて手動インクリメントする。
     - ② GitHub Web UI（`Actions` → `Caches`）または GitHub CLI（`gh cache delete <key>`）で該当キャッシュを手動削除する。
